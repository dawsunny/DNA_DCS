...
BMAP *bmap_create(dcs_u64_t size)
{
	...
	if(!(bmap = (BMAP *)malloc(sizeof(BMAP)))) {
        DCS_ERROR("bmap_create err: %d\n", errno);
		return NULL;
	}

	if(!(bmap->a = (dcs_u8_t *)calloc((size + CHAR_BIT-1)/CHAR_BIT, sizeof(dcs_s8_t)))) {
		free(bmap);
        DCS_ERROR("bmap_create err: %d\n", errno);
		return NULL;
	}
...
dcs_s32_t bmap_check(BMAP *bmap, dcs_s32_t n, ...)
{
	...
	for (i = 0; i < n; i++) {
		pos = va_arg(l, dcs_u64_t );
		if(!(GETBIT(bmap->a, pos % bmap->asize))) {
			return 0;
		}
	}
	va_end(l);
